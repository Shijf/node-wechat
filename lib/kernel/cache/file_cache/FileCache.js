 ...
 * @Author: shijf
 * @Date: 2020-06-15 19:39:52
 * @LastEditTime: 2020-06-21 12:33:08
 * @LastEditors: shijf
 * @FilePath: /node-echat/lib/kernel/cache/file_cache/FileCache.js
 * @Description: 
 */ 
'use strict';
const fs = require('fs');
...
class FileCache {
     ...
     */
    constructor(app) {
        let flag = app.config.cacheFlag;
        let regOfFlag = new RegExp('[\\\\/:*?\\"<>|]');
        if (regOfFlag.test(flag)) {
           let message = `${flag}中不能包含【\\\\/:*?<>|】这些非法字符,请修改合法的文件名!`;
           console.log(message);
            process.exit();
            // return flag;
        }
